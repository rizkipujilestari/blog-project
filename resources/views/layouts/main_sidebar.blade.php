...
<aside class="right-sidebar">
  <div class="widget">
    <legend class="widgetheading">Filter</legend>
      ...
      <form action="{{ url('blog') }}" method="GET" class="controls">
        ...
        <div class="control-group">
          ...
          <a href="{{ url('/blog') }}" class="btn btn-default">Reset</a>
        </div>
      </form>
  </div>    
  ...
  <div class="widget">
      ...
      <ul class="cat">
        @foreach ($categories as $cat)
          <li><i class="icon-angle-right"></i> <a href="{{ url('blog?category='.$cat->name) }}">{{ $cat->name }}</a><span> ({{ $cat->article_count }})</span></li>
        @endforeach
      </ul>
  </div>
